...
from multipage_classifier.datasets.transformer_dataset import (
    TransformerSample,
    TransformerDataset,
)
...
class MultipageTransformerPLModule(BaseLightningModule):
    ...
    def training_step(self, batch, *args):
        ...
        assert (
            len(image_tensors) == 1 and len(decoder_input_ids) == 1
    ...
    def eval_step(self, batch, *args):
        ...
        assert (
            len(image_tensors) == 1 and len(decoder_prompts) == 1
        ), "batch size > 1 not supported"
        ...
        for pred, answer, gt in zip(preds, answers, ground_truths):
            ...
            for truth, p in zip(gt, page_preds):
                out["acc_doc_id"].append(
                    truth.get("doc_id", None) == p.get("doc_id", None)
                )
                out["acc_doc_class"].append(
                    truth.get("doc_class", None) == p.get("doc_class", None)
                )
                out["acc_page_nr"].append(
                    truth.get("pag_nr", None) == p.get("page_nr", None)
                )
    ...
    def on_epoch_end(self, prefix: str):
        ...
        metrics[f"{prefix}/edit_distance"] = sum(flattend["scores"]) / len(
            flattend["scores"]
        )
        metrics[f"{prefix}/acc_doc_id"] = sum(flattend["acc_doc_id"]) / len(
            flattend["acc_doc_id"]
        )
        metrics[f"{prefix}/acc_doc_class"] = sum(flattend["acc_doc_class"]) / len(
            flattend["acc_doc_class"]
        )
        metrics[f"{prefix}/acc_page_nr"] = sum(flattend["acc_page_nr"]) / len(
            flattend["acc_page_nr"]
        )
    ...
    def validation_step(self, batch, *args):
       return self.eval_step(batch, *args)
